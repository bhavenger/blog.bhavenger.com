...
## Почему Vault?
[Vault](https://www.vaultproject.io) - хранилище секретов от признанных "решателей" проблем современной инфраструктуры - [Hashicorp](https://www.hashicorp.com), авторов Vagrant, Consul, Terraform, Otto, etc. Секреты хранятся в key-value виде. Доступ к хранилищу осуществляется исключительно через REST-API.
Основные фичи Vault:
...
```
Мастер-токен может создавать токены только с теми политиками, которыми он обладает. Это означает, что вам недостаточно выдать токену политику $service_name-prod-root,
```
path "secret/$service_name/prod/*" {  
  policy = "write"  
}
```
которая обладает полным доступом к secret/$service_name/prod/*, но также нужно озаботиться, политикой $service_name-prod-read,
```
path "secret/$service_name/prod/*" {  
  policy = "read"  
}
...
Вам придётся уравновесить каждую детализированную политику на чтение, чтобы этого не случилось. Например видоизменить политику $service_name-prod-root также
```
path "secret/service_name/prod/*" {  
  policy = "write"  
}
...
В документации к Vault зря не сказано о важности учёта выпущенных токенов.  Нет простого способа узнать, какие ключи присутствуют в системе. Если вы создали токен и забыли о нём, не записав никакой информации, то вам придётся дожидаться, пока у него закончится срок жизни. У root-токенов нет срока жизни, поэтому они останутся в вашей системе на бесконечность. Чтобы избавиться от этой ситуации, для создания нового токена в целях тестирования и проверки укажите прекрасный флаг -ttl="1h", который устанавливает время жизни. Это позволит спокойно работать и не бояться бесконтрольно увеличить количество токенов. Также есть риск не уследить за истечением срока жизни токена и узнать об этом во время деплоя, например. Эту проблему можно решить, записывая токены и мониторя их срок жизни. Но записывать токены куда-то в одно место - небезопасно. Для решения этой проблемы с версии Vault 0.5.2 каждый токен после создания возвращает параметр accessor, зная который можно взаимодействовать с токеном, не зная его самого (отзывать, обновлять, добавлять политики), например
...
1. [Установим vault](https://www.vaultproject.io/intro/getting-started/install.html).
2. Укажем расположение Vault:
```$ export VAULT_ADDR='https://vault.service.consul:8200'```
3. Авторизуемся под root токеном:
```$ vault auth 82c5fb97-da1b-1d2c-cfd5-23fa1dca7c85```
4. Запишем наш секрет:
```
$ vault write secret/service_name/prod/database base=appname login=appname password=difficult_password
```
5. Запишем политику для чтения в файл service_name_prod_read.hcl:
```
path "secret/service_name/prod/database*" {  
  policy = "read"  
}
```
6. Создадим политику в Vault:
```vault policy-write service_name_prod_read service_prod_read.hcl``` 
7. Сгенерируем токен для чтения:
```
...
```
8. Запишем token_accessor для последующего аудита и мониторинга.
9. Проверим, что есть доступ на чтение:
```
...
Спасибо за потраченное время, надеюсь это было полезно. Готов ответить на ваши вопросы.